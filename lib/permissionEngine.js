...
class PermissionEngine extends Base {
  ...
  _checkRoles(userRoles, roles) {
    ...
    try {
      ...
      if (!userAttrIsValid || !rolesIsValid) throw new Error('Fail, roles or userRoles is empty, verify roles and try again');
      return true;
  ...
  canAccess(userRoles, rules) {
    ...
    try {
      ...
      for (const rule of rules) {
        ...
        else throw new Error(`Rule type ${typeof rule} not allow`);
...
export const ErrorPermissionEngine = Error;